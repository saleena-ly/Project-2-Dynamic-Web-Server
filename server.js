...
app.get('/state/:selected_state_abbreviation', (req, res) => {
	let currentState = String(req.params.selected_state_abbreviation);
	db.all('SELECT * FROM Consumption WHERE state_abbreviation = ?', [currentState], (err, rows) => {
		if (err) { console.log(err); }
			...
			rows.forEach(row => {
				...
				let yearTotal = 0;
				for (key in count) {
					for(i in count[key].length)
					{
						yearTotal += parseInt(count[key][i]);
						console.log("yearTotal: ", yearTotal);
						console.log("add ", parseInt(count[key][i]), "to year Total");
					}
				}

				tableData += `<td>${yearTotal}</td>`;
				tableData += '</tr>';
			});
			...
			ReadFile(path.join(template_dir, 'state.html')).then(template => {
				...
				//replace h2
				response = response.replace('<h2>Yearly Snapshot</h2>', `<h2>${currentState} Yearly Snapshot</h2>`);
